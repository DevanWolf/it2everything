...
# This enables an approximation of the Amiga filter.
global FILTER
FILTER = True

# This speeds up the mixer at the cost of clipping sounding horrible.
global QUICK_N_DIRTY_MIX
QUICK_N_DIRTY_MIX = False
...
class ModPlayer:
	...
	def dotick(self, dsp, dspspeed):
		...
		f = (11500/2 if STEREO else 11500) if self.filteron else dspspeed
		
		if QUICK_N_DIRTY_MIX:
			for i in xrange(len(arr)):
				arr[i] = (128+((arr[i]*2)>>8))&255
		elif FILTER:
			for i in xrange(len(arr)):
				p = arr[i]*2-m
				if p < 0:
